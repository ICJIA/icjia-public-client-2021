...
| INJ-12 | `data.ts` + `research.ts` (the build/static render path) never imported `safeUrl` — externally-sourced URL fields rendered raw on static pages: job/meeting `external`, app contributor, dataset source, article `doi`, publication `fileURL` (+ `localArticlePath`), unit + page/home clickthrough links | **Medium** | ✅ Fixed (`0.50.4`) — full field-by-field sweep mirroring the client shapers (`data:` image-src excluded) |
...
**Completed in `0.50.4` (INJ-12 — build-path `safeUrl` sweep):**
- `data.ts` **and** `research.ts` (the build/static render path) now import `safeUrl` and guard **every** externally-sourced link URL, mirroring the client shapers field-for-field: article `doi`, dataset `sources[].url`, app `contributors[].url` (research.ts); job + meeting `external[].url`, publication `fileURL` + a site-relative guard on `localArticlePath`, unit `url`, page + home clickthrough `url`, biography→unit `url` (data.ts). Image/`data:` `src` fields (Research-Hub base64 splash heroes, `CmsImage`, Strapi media `/uploads/`) were **deliberately excluded** so inline images keep working; optional fields use `x ? safeUrl(x) : x` so an absent URL stays absent (no spurious `"#"`). A **no-op for every legitimate URL**, verified by a clean 3530-page build + 216 tests — **build and client render paths are now at URL-scheme-validation parity**, closing the INJ-4…10 class on the surface that serves 99% of traffic.
