# Astro Migration Checklist — fable-v1.0 (probe-first live islands + hub GraphQL thin fetch — icjia.illinois.gov flagship, 2026-06-10)

> **Checklist version: fable-v1.0 (canonical / current). Continuously updated.**
>
> File renamed from `astro-conversion-checklist-v7.1.md` → `astro-conversion-checklist-fable-v1.0.md` on 2026-06-10 (model-lineage marker: increments from here are authored with the newer Fable model; the v7.x numbering freezes at v7.8). **fable-v1.0 documents the probe-first live-island upgrade**: every listing island bakes a `(count, latest-updated)` probe at build and skips its full collection fetch when the live probe matches; the researchhub live fetches move from REST (~105MB of inline base64 per page view — LIVE-1's unfixed sibling) to thin GraphQL; live swaps preserve build-extracted images; the meetings row-expand detects post-build EDITS via an `updatedAt` gate; the rebuild webhook spec gains update/delete events on BOTH Strapi admins; the dynamic swaps/filters announce to assistive tech; and the post-change axe/Lighthouse sweep caught (and lesson 9 records) the `safeUrl`-on-absent-url `'#'`-link regression + the events-calendar contrast miss. See "## Probe-first live islands + hub GraphQL thin fetch (2026-06-10, fable-v1.0 increment)" at the end.
>
...
>
> **⚠️ SUPERSEDED 2026-06-10 (de-serverless).** The purge-on-publish stack above is SSR-era history: `purge-cache.mjs`, `setCache`, `PURGE_SECRET`, and the cache tags were all REMOVED when the site went fully static (see `docs/STATIC-ISLANDS-MIGRATION.md`). Do **not** re-create this webhook on a future backend. The current equivalent is far simpler: each Strapi admin (agency + researchhub) gets one webhook that POSTs a **Netlify build hook** on entry **publish/unpublish/update/delete** (debounced) — no function, no secret header beyond the hook URL itself. Spec: `docs/nightly-rebuild.md`. The Strapi-5 caveat that webhook config lives inside the admin (and silently vanishes on a backend migration) still applies — re-create the BUILD-HOOK webhook there, not this one.
...
## De-serverless Phase 2 (2026-06-03) — live-islands + cache teardown (v7.2 increment)

1. **`pnpm build` CANNOT verify an Alpine island — the browser must.** Astro treats the `x-data="…"` attribute as an opaque string and never parses it, so a syntax/logic error in an island's inline JS builds clean and only fails at runtime. A live-island change is **not** "done" on a green build: serve `dist/` (`astro preview`) and confirm in a real browser (console clean + the expected network calls + the component's reactive state). The publications island was confirmed by reading `Alpine.$data(el)` (`fullLoaded`, `all.length` 150→1108) and asserting the 4 Strapi requests returned `200`.
2. **Regex backslashes get eaten inside the `x-data` template literal.** The island's JS lives in a backtick template string in the `.astro` frontmatter; `\d`/`\s` are stripped to `d`/`s` before the string reaches the attribute, silently breaking the regex. Use backslash-free equivalents (`[0-9]` for `\d`, `.split(' ').filter(Boolean)` for `\s+`) — or double-escape (`\\d`) **and** grep the emitted attribute to confirm. Backslash-free is the safer default.
3. **Publications can't be fetched from GraphQL — REST pager only.** Strapi GraphQL clips this archive (`limit:990` silently drops rows, `2000` errors), which is why `getAllPublications` uses `/publications/count` + 500-row `_start` slices. That pager is a verbatim port of the legacy Vue SPA's *client-side* method, so porting it back to the browser is proven and CORS-open (`access-control-allow-origin: *`). The live-island reuses it; it skips `deepSanitize` (the SiteImprove HTML filter) because the table renders via `x-text` (auto-escaped) and SiteImprove reads the **baked** HTML, not the client swap.
4. **zsh does NOT word-split an unquoted `$VAR` in `for f in $FILES`** (unlike bash). A loop that `sed`-edits a newline-joined file list silently passes the whole list as one path ("File name too long") and edits nothing — but any sibling `git rm` in the same block still runs, leaving dangling imports. Use `… | while read -r f; do …` (or `${(f)FILES}`) and **grep-verify the edit actually happened** before trusting it.
5. **A static response can't set CDN cache headers**, so every `setCache(Astro.response, …)` is a dead no-op post-`output:'static'`. Removing the 37 calls + `lib/cache.ts` + the `cacheTTL`/`keepWarm`/`renderStrategy` config is pure dead-code deletion — but watch for orphaned *explanatory comments* above the removed calls (only 2 here) and test files that import the removed exports (`config.test.ts`).
6. **Trailing slash on a static Netlify deploy is a hosting-layer behavior, not a build artifact.** With `trailingSlash:'always'`, `dist/news/index.html` exists but `astro preview` 404s on `/news` (slash-less) by design — it does not emulate Netlify's CDN. Under the old SSR model an edge function did the redirect (no static dir files existed); static is structurally different (the dir index exists), so Netlify's native directory redirect *should* cover it — **verify on the branch deploy**, it is not locally provable.

## Site-wide live-islands — design decision + recon lessons (2026-06-03, v7.3 increment)

Design spec: `docs/superpowers/specs/2026-06-03-site-wide-live-islands-design.md` — drop the live/baked **rationing** (there's no serverless cost to ration anymore) and make the **entire** site live-on-refresh on two shared cores (`live-list` + `live-detail`), generalizing the publications `fetchLive→swap` pilot. Phased: all lists/home first → verify on a real deploy → all `[slug]` detail pages.

1. **Only the publications island was ever live.** After Phase 1 (`output:'static'`), every other surface (home, `/news`, `/researchhub/*`, meetings) is frozen baked HTML. `HomeResearch` *looks* live but client-fetches `/api/home-research.json`, which is `prerender=true` (a build-time snapshot). **A client `fetch()` of a `prerender=true` endpoint is NOT live** — always confirm the fetch target hits Strapi, not a baked `/api/*.json`.
2. **`astro dev` masks build-time staleness.** In dev, page frontmatter (`getHome()`) and `prerender=true` endpoints re-run per request, so a refresh *does* pick up new Strapi content — but a built/deployed static site does **not**. Reproduce/verify any content-freshness bug against `astro build` + preview/deploy, never `astro dev`.
3. **REST is `ACAO:*`; GraphQL reflects only the prod origin.** Verified: both `agency.icjia-api.cloud` + `researchhub.icjia-api.cloud` GraphQL return `access-control-allow-origin: https://icjia.illinois.gov` *only* → a live GraphQL fetch would be CORS-blocked on a Netlify **preview** URL. REST returns `*` → works on prod, previews, localhost. **Use REST for all browser-live fetches.** (News collection is `/posts`, not `/news`.)
4. **`markdown.js` is jsdom-bound; `contentSanitizer.js` is client-portable.** `markdown.js` imports `jsdom` + `server-dom` (linkedom) to back DOMPurify server-side → can't ship to the browser as-is. `contentSanitizer.js` only imports `brokenLinks` (no jsdom/node built-ins), `dompurify` is browser-native, `markdown-it` is isomorphic. So detail-page live-render = extract the shared markdown-it **config** + reuse `contentSanitizer.js`, swapping only the DOM backing (jsdom server / native browser); same config + sanitizer ⇒ byte-identical output (enforce via the existing `contentSanitizer.parity` suite).
5. **Length-based swap misses edits/deletes.** The pilot swaps when `liveRows.length > baseline.length` (new items only). The shared `live-list` core must swap on a content **signature** (hash of `id+updatedAt` across the collection) so edits and deletions go live too.

## Site-wide live-islands — Phase A execution: foundation + news reference (2026-06-03, v7.4 increment)

Built subagent-driven on `feat/astro-researchhub-fixes`: shared cores (`signature.ts`, `live-list.ts`, `shapers/*`, all Vitest-tested) + `/news` as the FIRST wired surface (the reference). Commits `a6458fb`→`5266cee`. All build + unit green; **island behavior browser-UNVERIFIED** (sandbox has no browser/deploy).

1. **Bringing a shared TS core into an Alpine island = the `@astrojs/alpinejs` `entrypoint` (v0.5.0).** This repo's islands historically inline ALL logic as `x-data` strings — there was NO precedent for importing a shared module into Alpine. The supported bridge: `alpinejs({ entrypoint: '/src/lib/live/alpine-entry' })`, whose default export `(Alpine) => { Alpine.data('newsLive', (elId) => ({...})) }` runs before `Alpine.start()`; the island becomes just `x-data="newsLive('news-data')"`. THIS is the pattern for every live surface — register one `Alpine.data` factory per surface in the entrypoint, composing the shared `fetchCollection` + that surface's shaper. (A subagent first re-inlined the core into the x-data string instead — reintroducing duplication + a tz drift bug; rejected.)
2. **Island rows need a sortable ISO date key, NOT the display date.** First cut sorted live rows by the formatted display string (`localeCompare` on "May 1, 2026") → wrong chronological order + wrong featured-card `.slice(1)`. Carry `pd` (publicationDate ISO) on the row shape and sort by it.
3. **Fetch-once islands take live data DIRECTLY; the content-signature is a POLLING optimization, not needed here** (refines v7.3 #5). On a single fetch-per-load island, live data is authoritative — just assign `rows = live`; Alpine's keyed `x-for` (`:key="it.p"`) diffs efficiently, so a signature guard is redundant (and is meaningless unless the SSR baseline rows ALSO carry `id`/`updatedAt`, which they don't). `contentSignature` stays in `lib/live` for if/when polling is added.
4. **`pnpm build` + Vitest CANNOT prove an island works** (v7.2 #1, again). News is the canary: browser-verify on a Netlify preview (baseline paints → `/posts` 200 → swaps on new/edited/deleted post → filters/pagination intact → offline = baseline) BEFORE replicating the entrypoint pattern to the other 6 surfaces.
5. **REST collection names resolved from the repo's own `src/graphql/*.js` query roots** (+ the public schemas): news=`posts`, funding=`grants`, programs=`programs`, meetings=`meetings`, events=`events`, hub=`articles`/`datasets`/`apps`. Recorded in `src/lib/live/sources.ts`; confirm each `/count` → 200 + `ACAO:*` at the deploy gate.

## Live-detail fallback — render post-build slugs client-side (2026-06-05, v7.5 increment)

Spec: `docs/LIVE-DETAIL-FALLBACK.md`. New requirement (owner, non-negotiable): an author who adds a record in the CMS must see the live page **immediately**, before the nightly rebuild mints it. The smart-404 now fetches the record and **renders it client-side** (was a "being published" placeholder). Meetings slice built + browser-verified; the pattern is per-type `shaper + twin renderer + parity test + CSS import + registry entry`.

1. **`<script is:inline>{`…`}</script>` SILENTLY NEVER EXECUTES — and it had killed the smart-404 since it shipped.** `is:inline` emits the body **verbatim**, so the `{`…`}` JSX-expression wrapper ships as a literal `{ }` block containing a template *string*; the IIFE inside never runs. That is why a published-but-unbuilt slug always hit the hard 404. **An `is:inline` script body must be the literal JS to execute** — never wrap it in `{`…`}` (use `set:html` for dynamic values, like the `nf-detect` JSON). Bonus trap: code that *was* inside that template literal has doubled escapes (`/\\/$/`), which become `SyntaxError: Invalid regular expression flags` once unwrapped → fix to `/\/$/`. Verify inline-script execution in a **built+served** page, never `astro dev`.
2. **Reuse `renderToHtml`, don't stand up a second client markdown-it.** Split `markdown.js` → browser-safe `markdown-core.js` (markdown-it + plugins + DOM fixers + `contentSanitizer`, parameterized by a DOMPurify instance) + a Node wrapper (jsdom DOMPurify; build output byte-identical) + `markdown.client.js` (native-`window` DOMPurify). Same config + plugins ⇒ footnotes/headings/tables render identically; `markdown.client.js` bundles with **no jsdom** in the client (grep-confirmed on `dist/_astro`). Lazy-`import()` the renderer only on a content-detail 404 hit so plain 404s stay light.
3. **Lock the twin renderer to the real `.astro` component via the Container API.** `getViteConfig` (not bare `defineConfig`) lets Vitest import `.astro`; render the component with `experimental_AstroContainer` and diff vs the twin, normalizing the three INVISIBLE deltas: dev-only `data-astro-source-*` attrs (absent in prod builds), the stateful `cmstbl<n>` table-id counter (`contentSanitizer.js:208`, never reset), and tag/text-boundary whitespace. Build pages stay untouched; a drift-guard test pins duplicated shaper helpers to `data.ts`'s originals.
4. **The 404 is now the universal preview shell, so it needs each detail type's CSS + (TODO) its context bar.** `inlineStylesheets:'always'` means the 404 only carries what 404.astro imports → add `import '../styles/meetings.css'` (etc.) per supported type. `SiteContextBar` keys off the pathname and renders nothing for `/404` at build (+ its CSS is scoped), so the section nav strip is **absent on the transient view** — a known gap to close client-side later.
5. **Verify the fallback by moving the built page aside.** The build fetches live Strapi, so every real slug IS built — to exercise the fallback, `mv dist/news/meetings/<slug> /tmp`, serve `dist` with a Netlify-like 404 handler, and load the slug in a real browser (CORS `ACAO:*` works from localhost). Confirm content renders + `document.title` set, AND a random path still shows the plain 404 with the `detail-preview` chunk NOT loaded.
6. **All 7 types done (1.5.45); per-type quirks worth remembering.** The pattern (shaper + twin renderer + Container-API parity test + registry entry + CSS import) replicated cleanly via parallel subagents (each required to land a GREEN parity test). Findings: (a) **researchhub heroes are base64 data-URIs**, so the components take their base64 Alpine-island branch (no astro:assets transform) → byte-exact, NOT the raw-URL deviation that news-post splashes need. (b) **events title is `name`, not `title`** (the build passes `title={event.name}`). (c) `/news/` + `/news/press/` share one renderer (no `/news/press/[slug]` page). (d) research.ts pure helpers are module-PRIVATE (not exported), so a drift-guard can't import-and-equality-check them like the data.ts ones — pin them with behavioral reference-value tests instead. (e) **Two escapers needed to match Astro:** text escapes `& < > '` (Astro escapes `'`→`&#39;`); double-quoted ATTRIBUTE values escape only `& "` (Astro leaves `< > '` raw) — JS-expression attrs (`x-data` with `=>`/`<`) MUST use the attribute escaper. (f) Component-SCOPED `<style>` never applies on the `/404` shell (hashed `data-astro-cid-*`, built for `/404`) — same gap class as the context bar; only imported/global stylesheets reach the transient render.

## Live listings + publications detail + two Alpine build-traps (2026-06-05, v7.6 increment)

Detail fallback extended to **publications** (`/about/publications/` was missing from the 404 DETECT array → new pubs 404'd while showing in the live list). Listings made live for **events, funding, researchhub** (articles/datasets/apps) on the meetings pattern: a `window.__liveRows` registry in `alpine-entry` (exposes `fetchCollection` + a light row shaper) + a `this.all` swap in each island's inline `init()`. Browser-verified each by trimming the baked baseline and confirming the live fetch restored it. *(Pattern upgraded 2026-06-10: the swap is now PROBE-GATED — the full fetch only runs when a baked `(count, latest-updated)` probe mismatches the live API — and the hub fetchers moved off REST (~105MB of inline base64) to thin GraphQL. See the fable-v1.0 increment.)*

1. **A backtick inside an Alpine `x-data` COMMENT silently breaks the build.** `x-data={`{ … }`}` is a template literal; a comment like `re-derives via `shown`` (backticks!) terminates it early → esbuild `Expected ")" but found <ident>`. NEVER put backticks in x-data comments/strings. The error LOCATION esbuild reports is the cascade point (a later getter), not the real line — search for stray backticks.
2. **`import('…')` inside an `x-data` string can't be bundled OR path-resolved by Vite.** A funding agent put `import('../lib/markdown.client.js')` in the Alpine string (funding cards need `x-html` summary markdown for live rows); at runtime that path resolves against the PAGE url (404) and Vite never sees it to chunk it. Fix: do the dynamic import in the `alpine-entry` registry fetcher (a real module → proper lazy chunk), and have the island swap just assign `this.all`. Pattern for any list whose cards render markdown: render it in the fetcher, not the island.
3. **Vitest (174 green) does NOT catch either trap** — no test imports the `.astro` islands or `alpine-entry`. Islands are only proven by a real `astro build` + a browser. Always build + browser-verify a live-island change.
4. **Don't blank a field on swap that the baked rows already have.** Funding cards bind `x-html="summaryHtml"`; the client-safe row shaper can't render markdown (no `window` at import), so it carries raw `summaryMd` + empty `summaryHtml`. Swapping that in raw would WIPE existing rows' summaries — so the registry fetcher renders `summaryHtml` before the swap. Match the baked row's full shape, not just the new fields.
5. **Remaining detail types to add (same pattern, prevent whack-a-mole):** `/about/biographies/` (`biographies`), `/about/employment/` (`jobs`), `/about/units/` (`units`, +related bios), `/grants/programs/` (`programs`), and the four `pages`-backed routes `/about/`, `/grants/`, `/innovation-and-digital-services/`, `/irb/` (all resolve from the single `pages` collection by slug — one `page` renderer can serve all four prefixes). And remaining LIST surfaces: programs, employment, the home strips (HomeTabbed/HomeResearch — multi-collection, currently fetch a build-time `/api/home-research.json` snapshot). *(Resolution, for reuse on other sites: every detail type + the programs/employment lists landed in 0.50.0. The home strips went live in 0.50.0 and were then REVERTED to the baked snapshot in 0.50.1 — fetching full hub collections with inline base64 splash images cost ~105MB per home visit (LIVE-1). Home freshness now rides the content webhook rebuild instead. Lesson: a "make it live" sweep must check the per-view payload BEFORE shipping, not after.)*

## Post-build "404 flash" + live meeting-detail expand (2026-06-08, v7.7 increment)

Branch-deploy polish on the 0.50.0 smart-404, prompted by a user seeing the "404" paint for ~200ms before the live render.

1. **"Set `[data-nf-state='checking']` before paint" did NOT prevent the 404 flash.** The 0.48/0.50 design ran an early `is:inline` script (positioned *before* the numeral in source order) to gate the "404", the theory being the parser sets the attribute before the numeral paints. On real Netlify deploys the numeral still painted ~200ms before the hide took visual effect. **Lesson: never rely on "JS runs before first paint" to suppress unwanted content** — paint scheduling on a real network/browser is not guaranteed to honor source order vs. first paint. Make the unwanted state `display:none` in STATIC CSS and *reveal* it with JS, not the reverse. Here: hide the numeral + "not found" copy by default; reveal only on `[data-nf-state='gone']` (confirmed miss / non-detail path / `<noscript>` for JS-off). A candidate paints the neutral "checking" state, never "404", independent of timing.
2. **`<style is:inline>` inside `<noscript>` survives Astro's style hoisting.** A plain `<style>` is hoisted to `<head>` (which would break a JS-off gate); `is:inline` keeps it verbatim, in place inside the `<noscript>`. Use it for any state CSS that must live inside `<noscript>`.
3. **An interactive list's row-expand needs its OWN live fallback, separate from the live list.** Making the meetings LIST live (`__liveRows`) shows a post-build meeting, but expanding it still fetched the baked `/api/meeting/<slug>.json` (404). Added a parallel `window.__liveDetail.meeting(slug)` registry (single-record Strapi fetch + the existing `shapeMeeting`) that the inline `loadDetail` falls back to. **Live list AND live detail-on-expand are two separate post-build gaps** — closing one doesn't close the other.
4. **rAF/computed-style probing is strong for STATE assertions, weak for FLASH detection.** A 45-frame `requestAnimationFrame` sampler reading `getComputedStyle().display` returned `everNumVisible:false` and did NOT disprove a real ~200ms flash (sub-frame paints slip between samples). Trust the human's eyes for a reported flash; verify the FIX *structurally* (the content is `display:none` by default in the built artifact) rather than by sampling.

## Build/client shaper parity is a SECURITY surface (red/blue audit, 2026-06-08, v7.8 increment)

Audited the 0.50.2 change (404 inversion + live meeting-detail) with an independent red-team agent. The change was clean, but the audit surfaced a class trap worth carrying forward.

1. **A security fix applied to the CLIENT shapers must ALSO be applied to the BUILD shapers (`data.ts`).** The 0.50.1 `safeUrl` scheme-guard (block `javascript:`/`data:` in hrefs) went into `src/lib/live/shapers/*` (the live/client path) but `data.ts` (the build/static path) never imported `safeUrl` — so the **static pages + baked JSON**, which serve 99% of traffic, still rendered raw CMS URLs. **Two render paths = two places to fix.** When you harden one shaper, grep the twin (`data.ts` ↔ `lib/live/shapers/`) and the `*.drift.test.ts` that locks them, and fix both. (Done in 0.50.4 — full field-by-field sweep across `data.ts` + `research.ts`, mirroring the client shapers; `data:` image-src fields excluded.)
2. **A `safeUrl` sweep must be field-specific, NOT blanket** — `safeUrl` blocks `data:`, and some fields legitimately carry `data:` (base64 images, e.g. Research-Hub splash heroes). Apply it only to **link/href** URL fields (external links, file URLs, contributor/unit/page links), never to image `src` fields. The client fix did this field-by-field with parity tests; mirror that on the build path.
3. **A "hidden-by-default until JS reveals it" UI needs a JS-independent fallback.** Inverting the 404 to hide the error until a confirmed miss makes the reveal depend on JS — and the revealing logic (the resolver's 6s timeout) lived in a separately-bundled chunk that can fail to download. A watchdog in the INLINE script (which always runs during parse) keeps the page from getting stuck. Belt-and-suspenders for any default-hidden state whose un-hide lives in a lazy chunk.
4. **`encodeURIComponent` on a slug that flows into a Strapi REST query is load-bearing security, not cosmetics.** The backend honors `_publicationState=preview` / `_limit` if smuggled (verified: `/meetings/count` 290 vs 288 with preview); the encode is the only thing stopping a crafted URL path from injecting REST params or reaching drafts. Never drop it; never build the query by raw concatenation.


## Probe-first live islands + hub GraphQL thin fetch (2026-06-10, fable-v1.0 increment)

Upgrade of the v7.6/v7.7 live-island pattern, prompted by an external assessment of the live-data layer: the islands were downloading **entire collections on every page view** to discover (almost always) that nothing changed — including a ~105MB hub-articles read that was LIVE-1's unfixed sibling — and the freshness story had a hole (post-build **edits** to built detail pages). All fixes are additive; the baked-baseline fallback semantics are unchanged.

1. **A live island that downloads the whole collection "to check freshness" is a per-view tax — probe first.** Bake the collection's `(count, latest-updated stamp)` into the island at build (a `data-live-probe` attr; compute it in the COMPONENT frontmatter — Astro components can `await` — so no page-level prop threading); on `init()`, re-read those two values live (REST: `/count` + `?_sort=updated_at:DESC&_limit=1`, two tiny parallel GETs; GraphQL: ONE POST with `<coll>Connection{aggregate{count}}` + `sort:"updatedAt:desc", limit:1`) and run the full fetch only on mismatch. Detection is complete: add→count, edit→stamp (Strapi bumps `updated_at` on every save), delete→count. Numbers here: `/news` ~807KB → ~2KB per unchanged view; funding also skips 107 client-side markdown renders; hub articles ~105MB → ~1KB.
2. **Probe and full fetch MUST observe the same data source** — same transport, same status filters — or a permanent probe-mismatch silently degrades to full-fetch-every-view. The two Strapi instances here disagree on timestamp casing (agency REST/GraphQL: snake_case `updated_at`; hub GraphQL: camelCase `updatedAt`) → normalize `updated_at ?? updatedAt` on BOTH sides. Failure semantics: the probe may only SKIP work when it PROVES freshness; any probe failure (either side null) → full fetch. Never invert this.
3. **Never client-fetch a Strapi collection whose REST records inline base64 — and MEASURE the per-view payload before shipping any live fetch.** Hub articles REST returned ~105,671,287 bytes for 252 records (base64 splash/thumbnail/images ride every record; v3 REST has no field selection) even though the row shaper kept ~12 thin fields — the download + `JSON.parse` happen regardless. Fix: thin GraphQL selecting exactly the build's own group-query field set (`graphql/hub.js`), so live rows can't drift from baked rows. Hub GraphQL quirks: `where` VARIABLES are silently ignored (inline all args — pre-existing repo lesson) and Strapi GraphQL clips big collections (agency publications drop rows at `limit:990`) → page at ≤500 with `start` offsets and de-dupe by id.
4. **A live swap must not blank build-only fields — merge them back by slug.** Generalizes v7.6 #4 (funding `summaryHtml`): hub rows carry build-extracted `imagePath`/`hasImg` that no client fetch can reproduce; live-shaped rows had `ip:null`/`hasImg:false`, so every swap visually DELETED all article thumbnails + app images. `mergeBakedHubImages(live, baked)` copies the baked image fields onto live rows by slug; live owns membership (deleted records don't resurrect); genuinely-new rows stay imageless until the rebuild (the accepted transient). Count labels must bind to `all.length`, not the server-baked total, for the same reason.
5. **There are THREE post-build freshness gaps, not two: live LIST, live detail-on-MISS, live detail-on-EDIT.** v7.7 closed list + miss (404 fallback / expand fallback). The third: a record EDITED after the build serves its stale built page / baked JSON until the next rebuild, while the live list shows the new title — a visible inconsistency. Close it with an `updatedAt` gate: bake the stamp into the per-slug detail JSON, carry it on live rows (the probe-triggered swap supplies it), and on expand compare `row.updatedAt > json.updatedAt` (plain ISO-string compare — same Strapi field, same format) → fetch the live record via the existing `__liveDetail` path. The webhook (next lesson) shrinks the edit-staleness window to minutes for every type; the gate covers those minutes for the highest-stakes surface (meeting agendas).
6. **Webhook events: publish-only means EDITS never trigger a rebuild.** Strapi v3 fires `entry.update` — NOT publish — when an already-published entry is edited, so a publish/unpublish-only build-hook webhook leaves an edited agenda or corrected meeting date stale on built pages for up to 24h (nightly). Subscribe **publish/unpublish/update/delete** (+ media if uploads matter), debounce the burst, and configure the webhook on EVERY Strapi admin that feeds the site — this site has TWO (agency + researchhub) and the second is easy to forget. Spec updated: `docs/nightly-rebuild.md`.
7. **An island that ships a SUBSET must apply the same filter on swap.** `/irb/irb-meetings/` baked irb-only rows into the shared `#meetings-data` island but mounted `MeetingTable` without `category` — so the (previously unconditional) live swap replaced the IRB table with ALL 288 meetings. Wherever an island's baked rows are pre-filtered, the swap path needs the same filter prop. Found by reading the wiring, not by tests — islands still need build + browser verification (v7.6 #3 stands).
8. **Dynamic swaps are invisible to screen readers — and REPLACING a node replaces (silences) its `aria-live` region.** The 404 preview did `sec.outerHTML = preview.html`, wiping the very region that said "checking…" — an SR user heard nothing further. Pattern: ONE body-level `.sr-only` polite region, created BEFORE any swap and reused for all announcements (`window.__liveAnnounce`); announce live swaps and user-driven list changes (filter/tab/page); on a client-rendered "navigation" (the 404 preview), also move focus to the rendered content's first heading (`tabindex="-1"` + `focus()`). None of this is catchable by axe — it audits static DOM; interaction-time semantics need manual/scripted verification.
9. **A blanket security sweep is itself a change — regression-test its UI/a11y surface (the `safeUrl`-on-absent-url trap).** The INJ-12 sweep (0.50.4) applied `safeUrl(x.url)` to every external-URL field uniformly — but `safeUrl(undefined)` returns its `'#'` neutralization fallback, so **fields that legitimately had NO url got a minted `'#'`**: every url-less app contributor became `<a href="#">`, a bare-STRING CMS contributor became a **nameless empty link** (axe `link-name`, WCAG A, found by the 2026-06-10 sweep), and url-less dataset sources became dead `'#'` links instead of plain text. Rules: (a) `safeUrl`'s fallback is for **present-but-dangerous** values — call it ONLY behind an existence check (`url ? safeUrl(url) : …`); (b) normalize multi-shape CMS fields (the hub stores contributors as BOTH `{title,url}` objects and bare strings — `{...c}` on a string spreads index chars) at the shaper so templates can't meet a shape they don't handle; (c) after any sweep that rewrites rendered fields, re-run axe on the surfaces that render them — the 5/31 "0 violations" sweep predated the regression, and the events-calendar contrast miss (`#b0b0b0` on `#fafafa` ≈ 2.2:1) similarly shipped in a feature built AFTER the last sweep. Audits are point-in-time; re-sweep after feature/security changes that touch rendered output.
