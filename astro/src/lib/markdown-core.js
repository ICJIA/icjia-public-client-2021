// Browser-safe markdown → sanitized-HTML CORE. Shared by:
//   - markdown.js          (Node/build wrapper: jsdom-backed DOMPurify + server-dom shim)
//   - markdown.client.js   (browser wrapper: native-window DOMPurify, native DOMParser)
//
// Extracted verbatim from the original markdown.js so the rendered output is
// byte-identical in both environments — same markdown-it instance, same plugin
// set (footnotes, heading anchors, tables, figures, attrs), same DOM fixers, same
// contentSanitizer. The ONE environment-specific dependency (DOMPurify's backing
// window: jsdom in Node, the real window in the browser) is injected by the
// caller via createMarkdownRenderer(DOMPurify), so this module imports NO jsdom
// and NO server-dom. The DOM fixers + contentSanitizer read the global
// `DOMParser` at call time (native in the browser; linkedom-shimmed in Node by
// the wrapper's `import "./server-dom"`).
...
// Heading extraction (h2) for the page TOC. Pure markdown-it + DOMParser — no
// DOMPurify — so it lives at module scope (shared by both wrappers).
...
// Factory: bind the markdown pipeline to a caller-provided DOMPurify instance
// (jsdom-backed in Node, native-window in the browser). Returns the same public
// API the original markdown.js exported.
const createMarkdownRenderer = function (DOMPurify) {
  ...
  return { renderToHtml, renderInline, parseHeadings };
};

export { createMarkdownRenderer, parseHeadings };
